...
白云截屏的长图合并不是把多张图片简单上下拼接，而是使用我们持续维护的[白云长截屏合并算法方案](https://github.com/touensan/whiteyun-long-screenshot-algorithms)：先识别稳定的系统边缘，再进行重叠搜索、分块视觉一致性判断、重复内容消歧和安全接缝选择，最后通过流式输出与失败保护生成结果。
