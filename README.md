...
白云截屏的长图合并不是把多张图片简单上下拼接，而是使用我们持续维护的[白云长截屏合并算法方案](https://github.com/touensan/whiteyun-long-screenshot-algorithms)：先识别稳定的系统边缘，再进行重叠搜索、分块视觉一致性判断、重复内容消歧和安全接缝选择，最后通过流式输出与失败保护生成结果。算法的[公开更新日志](https://github.com/touensan/whiteyun-long-screenshot-algorithms/blob/main/CHANGELOG.md)会持续记录每次重要迭代。

这套方案已经在大量 Android 长截屏实践中迭代验证，在常见的连续滚动页面上大多数情况下都能稳定工作；遇到重复卡片、动画、低纹理或证据不足的页面，会保留候选并提示复核，而不是静默输出一张可能错位的长图。算法仓库会长期记录迭代、实验结果和社区改进，欢迎对计算机视觉、图像处理和跨平台实现感兴趣的开发者参与。
...
白云截屏默认识别并跟随 Android 系统语言。首页右上角的翻译图标（A/文）可以直接打开语言选择器，也可以在 **设置 → 切换语言** 中选择；选择“跟随系统”即可恢复自动识别。

目前包含 19 种语言：英语、简体中文、繁体中文、西班牙语、法语、德语、巴西葡萄牙语、俄语、日语、韩语、阿拉伯语、印地语、印度尼西亚语、意大利语、土耳其语、越南语、泰语、波兰语和荷兰语。

所有界面翻译均随应用打包，运行时无需联网。应用桌面名称也会跟随系统或当前应用语言切换；启动图标使用 Android 自适应图标的橙色背景与原始白云截屏标识，由 Pixel、OnePlus 等桌面自动裁成圆形或圆角方形，不会出现形状套形状。欢迎母语使用者提交校对与措辞改进。
